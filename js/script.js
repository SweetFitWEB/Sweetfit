...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function loadPage(page, text) {
    ...
    fetch(`http://localhost:8000/views/${page}`)
      .then(res => res.text())
      .then(html => {
    ...
    .then(data => {
      ...
      let html = `
        <div class="ticket-header">
          <img src="img/pinchesHamburguesas.png" class="only-print" alt="Logo">
          <h2>PINCHES HAMBURGUESAS-VENTA</h2>
        </div>
    ...
    .then(data => {
      ...
      let html = `
        <div class="ticket-header">
          ...
          <h2>PINCHES HAMBURGUESAS-COMPRA</h2>
          <p class="ticket-date">${fechaFormateada}</p>
...
function inicializarSelectoresFecha() {
  ...
  function actualizarSelectorVisible() {
    const tipo = tipoReporte.value;

    // Oculta todos los inputs de fecha
    fechaDia.style.display = "none";
    fechaSemana.style.display = "none";
    fechaMes.style.display = "none";

    // Muestra solo el que corresponde
    if (tipo === "diario") {
      fechaDia.style.display = "inline-block";
    } else if (tipo === "semanal") {
      fechaSemana.style.display = "inline-block";
    } else if (tipo === "mensual") {
      fechaMes.style.display = "inline-block";
    }
  }

  // Ejecutar al inicio para mostrar el correcto por defecto
  actualizarSelectorVisible();

  // Actualizar al cambiar el tipo
  tipoReporte.addEventListener("change", actualizarSelectorVisible);
}
...
function inicializarReportes() {
  ...
  function actualizarTitulo(tipo) {
    switch (tipo) {
      case "diario": return "Diarias";
      case "semanal": return "Semanales";
      case "mensual": return "Mensuales";
      default: return "";
    }
  }
  ...
  function formatearFecha(fechaStr, tipo) {
    ...
    const fecha = new Date(Date.parse(fechaStr)); // ahora sí entiende bien ese formato
  ...
  async function obtenerDatosVentas(tipo, fecha) {
    try {
      ...
      if (tipo === "semanal") {
        ...
        for (let v of ventas) {
          const dia = v.fecha.toLocaleDateString("es-MX", { weekday: "long" }).toLowerCase();
        ...
        for (let dia of diasSemana) {
          labels.push(capitalizar(dia));
          data.push(ventasPorDia[dia] || 0);
        }
        ...
        if (ventas.length === 0) {
          return { labels: ["Semana 1", "Semana 2", "Semana 3", "Semana 4", "Semana 5"], data: [0, 0, 0, 0, 0] };
        }

        // Función para obtener la semana del mes (1 a 5)
        function obtenerSemanaDelMes(fecha) {
          const primerDiaMes = new Date(fecha.getFullYear(), fecha.getMonth(), 1);
          const diaDelMes = fecha.getDate();
          const ajuste = primerDiaMes.getDay(); // Domingo=0...Sábado=6
          return Math.ceil((diaDelMes + ajuste) / 7);
        }

        // Agrupar ventasa por semana del mes
        const ventasPorSemana = {};
        for (let v of ventas) {
          const semana = obtenerSemanaDelMes(v.fecha);
          ventasPorSemana[semana] = (ventasPorSemana[semana] || 0) + v.total;
        }

        // 5 semanas máximo para cubrir meses que se extienden (a veces 4, a veces 5)
        const labels = ["Semana 1", "Semana 2", "Semana 3", "Semana 4", "Semana 5"];
        const data = labels.map((_, i) => ventasPorSemana[i + 1] || 0);

        return { labels, data };

      } else {
        return {
          labels: ventas.map(v => formatearFecha(v.fecha.toISOString(), tipo)),
...
function inicializarExportacionReportes() {
  ...
  if (!btnCSV || !btnPDF || !tipoReporte) return;

  async function obtenerDatosDetalles() {
    const tipo = tipoReporte.value;
    const fecha =
      (tipo === "diario" && document.getElementById("fechaDia")?.value) ||
      (tipo === "semanal" && document.getElementById("fechaSemana")?.value) ||
      (tipo === "mensual" && document.getElementById("fechaMes")?.value) || "";

    try {
      const res = await fetch(`http://localhost:5000/api/reportes/detalles?tipo=${tipo}&fecha=${encodeURIComponent(fecha)}`);
      if (!res.ok) return null;
      const json = await res.json();
      if (!json || !json.ventas || !json.compras) return null;
      return json;
    } catch {
      return null;
    }
  ...
  function exportarCSV(data) {
    ...
    let csv = "";
    csv += "PINCHES HAMBURGUESAS\n";
    csv += "Reporte Detallado de Ventas y Compras\n";
    csv += `Fecha de generación: ${fechaHoy}\n`;
    csv += "\n";
    ...
    csv += "SECCIÓN: VENTAS\n";
    csv += "===========================\n";
    csv += "Tipo,Fecha,Producto,Categoría,Cantidad,Precio Unitario,Subtotal\n";
    ventas.forEach(v => {
    ...
    csv += "\n";
    csv += "===========================\n";
    csv += "SECCIÓN: COMPRAS A PROVEEDORES\n";
    csv += "===========================\n";
    ...
    csv += "TOTALES\n";
    csv += "===========================\n";
    csv += `Total Ventas,,,$${Number(totalVentas).toFixed(2)}\n`;
    csv += `Total Compras,,,$${Number(totalCompras).toFixed(2)}\n`;
    ...
    link.href = url;
    link.download = `reporte_${tipoReporte.value}.csv`;
    link.click();
  ...
  async function exportarPDF(data) {
    ...
    // Obtener imagen base64 del logo
    const logoBase64 = await convertirImagenABase64("img/pinchesHamburguesas.png");
    ...
    // Encabezado con logo y nombre del negocio
    doc.addImage(logoBase64, "PNG", 14, 10, 25, 25);
    doc.setFontSize(20);
    doc.setFont("helvetica", "bold");
    doc.text("PINCHES HAMBURGUESAS", 45, 20);
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text("Reporte Detallado de Ventas y Compras", 45, 28);
    doc.setFontSize(10);
    doc.text(`Fecha de generación: ${fechaHoy}`, 45, 34);
    ...
    if (ventas.length > 0) {
      ...
      doc.autoTable({
        startY: y,
        head: [["Fecha", "Producto", "Categoría", "Cantidad", "Precio", "Subtotal"]],
        body: ventas.map(v => [
          v.FECHA_VENTA,
          v.producto,
          v.CATEGORIA,
        ...
        theme: "grid",
        styles: { fontSize: 9 },
        headStyles: { fillColor: [255, 102, 0] }, // Naranja
    ...
    if (compras.length > 0) {
      ...
      doc.autoTable({
        startY: y,
        head: [["Fecha", "Proveedor", "Producto", "Cantidad", "Subtotal"]],
        body: compras.map(c => [
          c.FECHA_COMPRA,
          c.proveedor,
          c.producto,
        ...
        theme: "grid",
        styles: { fontSize: 9 },
        headStyles: { fillColor: [0, 153, 204] }, // Azul
        margin: { left: 14, right: 14 }
      });
      y = doc.lastAutoTable.finalY + 10;
    }
    ...
    // Totales
    doc.setFontSize(12);
    doc.text(`Total de Ventas: $${Number(totalVentas).toFixed(2)}`, 14, y);
    y += 7;
    doc.text(`Total de Compras: $${Number(totalCompras).toFixed(2)}`, 14, y);
    y += 15;

    // Pie de página
    doc.setFontSize(10);
    doc.setTextColor(120);
    doc.text("Gracias por confiar en PINCHES HAMBURGUESAS", 105, 285, { align: "center" });
    doc.text("Contacto: 55-1234-5678 | IG: @pincheshamburguesas", 105, 290, { align: "center" });

    doc.save(`reporte_${tipoReporte.value}.pdf`);
  }

  btnCSV.addEventListener("click", async () => {
    const datos = await obtenerDatosDetalles();
    if (datos) exportarCSV(datos);
  });

  btnPDF.addEventListener("click", async () => {
    const datos = await obtenerDatosDetalles();
    if (datos) await exportarPDF(datos);
  });
}
